...
doLedCmd(char *cmd)
{
  ...
  Assert(cmd[1] == ',');
  char *s = &cmd[2];
  UInt32 redDir       = getNumber(&s);  Assert(s[-1] == ',');
  UInt32 redDutyCycle = getNumber(&s);  Assert(s[-1] == ',');
  UInt32 blueDir      = getNumber(&s);  Assert(s[-1] == ',');
  UInt32 bluDutyCycle = getNumber(&s);  Assert(s[-1] == ',');
  Assert(s == (cmd + strlen(cmd)));
  pwmSet(redDir, redDutyCycle,
	 bluDir, bluDutyCycle);
}

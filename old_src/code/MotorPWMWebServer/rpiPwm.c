...
void
setMotorDirectionA(int d)
{
  Assert((d == 0) || (d == 1));
  // printf("setMotorDirection(%d)\n", d);
  if (d == 0) {
    g.gpioRegPtr[GPCLR0] = (1 << 5);
    g.gpioRegPtr[GPSET0] = (1 << 6);
  } else {
    g.gpioRegPtr[GPCLR0] = (1 << 6);
    g.gpioRegPtr[GPSET0] = (1 << 5);
  }
}

void
setMotorDirectionB(int d)
{
  Assert((d == 0) || (d == 1));
  if (d == 0) {
    g.gpioRegPtr[GPCLR0] = (1 << 7);
    g.gpioRegPtr[GPSET0] = (1 << 8);
  } else {
    g.gpioRegPtr[GPCLR0] = (1 << 8);
    g.gpioRegPtr[GPSET0] = (1 << 7);
  }
}

static void
pwmSet(UInt32 redDir, UInt32 redDutyCyclePercent,
       UInt32 bluDir, UInt32 bluDutyCyclePercent)
{
  UInt32 freq;
  switch (g.modelNum) {
  ...
  UInt32 rangeRed = freq / 18000;
  UInt32 rangeBlu = freq / 18000;
  ...
  setMotorDirectionA(redDir);
  setMotorDirectionB(bluDir);
